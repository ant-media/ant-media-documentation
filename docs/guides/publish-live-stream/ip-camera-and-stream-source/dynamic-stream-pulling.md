...
- Go to the Ant Media web panel and create a broadcast with the type Stream Source or IP Camera. To enable the feature for the stream, check the `Auto Start/Stop Streaming` checkbox as shown below.
...
- After creation, Ant Media Server will start pulling the stream automatically and broadcast status will turn to `Broadcasting`.
  ...
  ![](@site/static/img/dynamic-stream-pulling-2-broadcasting.png)

- Alternatively, you can also activate Auto Start/Stop Streaming for an existing broadcast by modifying its settings using the REST API. To do that, send a `PUT` request using the [Update Broadcast Rest API](https://antmedia.io/rest/#/default/updateBroadcast).
...
- Open a new tab and start watching the live stream using the below URL. Now the server will start fetching streams.
...
- Close the player tab. Since there are no viewers anymore, the Ant Media Server will stop pulling the stream within a few seconds, and the broadcast status will change to `Offline`.
  ...
  ![](@site/static/img/dynamic-stream-pulling-3-offline.png)
