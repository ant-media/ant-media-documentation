...
description: Decide which Ant Media Server Azure path fits your team—cluster setup, ARM templates, or supporting load balancer and upgrade guides.
...
4. **[Upgrade Scale Set](/guides/clustering-and-scaling/azure/upgrade-azure-cluster/)** — supporting guide for upgrading an existing Azure Scale Set (VMSS) cluster.
...
| **Upgrade Scale Set** | Existing VMSS clusters that need upgrades | Supporting |

## Can I start with one and move to another?

Yes. Many teams start with a guided cluster setup, then standardize on the **ARM template** for the next environment. Load balancer and upgrade docs apply to an existing cluster—they are not alternate “products.”
