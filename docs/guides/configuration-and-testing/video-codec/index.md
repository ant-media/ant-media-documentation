...
description: Overview of H.264, VP8, H.265, and AV1 video codecs in Ant Media Server and how to choose the right one.
...
Ant Media Server supports **H.264**, **VP8**, **H.265 (HEVC)**, and **AV1** for live streaming. The codec you enable affects browser compatibility, bandwidth use, latency, and which protocols you can use.
...
| [AV1](/guides/configuration-and-testing/video-codec/av1/) | Yes (browser-dependent) | Yes | Newest option; best compression, higher encode cost. |

WebRTC publishing and playback rely on **H.264**, **VP8**, and **AV1** depending on browser and device. **H.265** is mainly for **HLS**, **LL-HLS**, and **CMAF (DASH)** delivery where you want lower bitrate at the same quality.
...
| Cutting-edge, bandwidth-limited WebRTC | **H.264 + AV1** | AV1 can reduce bitrate, but test on your target browsers and hardware first. |
...
- [AV1](/guides/configuration-and-testing/video-codec/av1/)

## Codec configuration

In older Ant Media Server versions, only **H.264** and **VP8** could be toggled from basic application settings; **H.265** required advanced settings, and **AV1** was not available.

Starting with **Ant Media Server v3.0.1**, all supported video codecs (**H.264**, **H.265**, **VP8**, and **AV1**) can be enabled from **Application → Settings** in the web panel.
