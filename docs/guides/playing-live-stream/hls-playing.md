...
```bash
curl -X POST -H "Accept: Application/json" -H "Content-Type: application/json" http://<Your-Ant-Media-Server>/live/rest/v2/broadcasts/create -d '{"streamId":"test1","name":"test1s","type":"liveStream","hlsParameters":{"hlsTime":"4","hlsListSize":"7","hlsPlayListType":"event"}}'
```
...
```http(s)://AMS-domain-or-IP:Port/live/streams/StreamId.m3u8```

If adaptive bit rates are enabled in the application (Enterprise Edition), the HLS (.m3u8) URL will be as follows:

```http(s)://AMS-domain-or-IP:Port/live/streams/StreamId_adaptive.m3u8```
