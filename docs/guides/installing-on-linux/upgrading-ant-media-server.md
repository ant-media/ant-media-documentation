...
Make sure you have SSH access to your AMS instance with sudo/root privileges, and know your deployment type below — it determines which path fits you.

```mermaid
flowchart TD
    A{Cloud Marketplace instance? AWS, Azure, GCP}
    A -->|Yes| B{Need to keep existing settings/data?}
    ...
    A -->|No, self-hosted| E{Need a specific version, not latest?}
...
The script checks your current version against the latest available, and downloads and installs the update if one is needed. It automatically detects whether you're on Community or Enterprise and upgrades accordingly — the same script works for both. For more detail on what it does, see the [script source](https://github.com/ant-media/Ant-Media-Server/blob/master/src/main/server/upgrade.sh).
...
- **Community Edition**: download it from the [GitHub Releases page](https://github.com/ant-media/Ant-Media-Server/releases).
