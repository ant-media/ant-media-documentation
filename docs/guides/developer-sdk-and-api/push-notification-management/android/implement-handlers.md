...
description: Create Firebase messaging service, call notification handlers, and WebRTC integration classes.
keywords: [Android Push Handlers, FirebaseMessagingService, Call Notification, Ant Media Server Documentation]
sidebar_position: 5
...
Create the classes that handle incoming push notifications, call events, and WebRTC interactions. These examples show an incoming-call notification flow with Accept/Decline actions.

After implementing these classes, register the device token and send notifications — see [Send Notifications](/guides/developer-sdk-and-api/push-notification-management/send-notifications/).

## Ant Media Firebase Messaging Service

Extends `FirebaseMessagingService` to handle incoming FCM messages and token updates.
