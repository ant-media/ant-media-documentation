...
Each NDI source is picked up by exactly one AMS application, mapped through the `ndiSources` parameter in that application's `red5-web.properties` file. Set it from the web panel rather than editing the file directly:

1. Go to the application's **Settings → Advanced** page. `ndiSources` isn't listed there by default, but AMS lets you add any property that isn't — doing so writes it into `red5-web.properties`, the same as editing the file by hand.
2. Add a new property with key `ndiSources`, mapping the NDI source name from Step 2 to the stream ID you want AMS to publish it as:

```
{"<SENDER_HOSTNAME> (Test Pattern)": "<STREAM_ID>"}
```

See [AMS Application Configuration](/guides/configuration-and-testing/ams-application-configuration/#adding-additional-settings) for the general steps on adding settings this way.
